...
double Rational::printd(bool output) //Print function that uses a bool
{
	...
	{
		if (output == true)
			cout << numerator / static_cast<double>(denominator) << endl; // Converts the fraction to a double decimal.
		return numerator / static_cast<double>(denominator);
	}
	else // Honestly not sure if there is ever an instance when this will happen, but in the case it does, the fraction will be printed in form num / denom.
	{
		if (output == true)
			cout << numerator << " / " << denominator << endl;
		return numerator / static_cast<double>(denominator);
...
/*bool Rational::operator==(Rational& R) // equal to comparison operator overloading
{
...
//Make compare or less than..  integer to numberator to clear up these last two errors. or create
//a left hand side and right hand side to make both objects..then the initial == would work.
...
bool Rational::operator==(int x, Rational R)
{
...
bool Rational::operator<=(Rational& R) // less than or equal to comparison operator overloading
{
	if (this->printd(false) <= R.printd(false))
	{
		return true;
